		...
		<>
			<div className={`fixed transition-all ${closeOverlay} inset-0 bg-black/75 z-40`}>
				...
				<div className={`fixed overflow-y-auto z-50 p-4 delay-200 transition-all duration-300 overflow-hidden ${closeProjectPreview ? '-left-20 w-0' : 'w-10/12 md:w-4/12'} inset-y-0 bg-white/30 backdrop-blur-md`}>
					...
					<article>
						<h1 className={`mt-4 ${loading && 'p-4 bg-white/60 animate-pulse'} transition-all ${closeProjectPreview && '-translate-x-full'} text-white font-mono font-bold text-xl md:text-lg`}>{ !loading && context?.data.projectName }</h1>
						<p className={`mt-2 ${loading && 'p-2 bg-gray-500 animate-pulse'} transition-all ${closeProjectPreview && '-translate-x-full'} text-white md:text-sm font-light`}>{ !loading && context?.data.projectDesc }</p>
